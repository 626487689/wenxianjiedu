...
export class ObsidianMarkdownWriter implements MarkdownWriter {
  ...
  async write(outputDir: string, input: MarkdownOutputInput): Promise<OutputWriteResult> {
    ...
    const frontmatter = this.buildFrontmatter(input)
    const content = this.buildContent(input)

    const fullContent = `${frontmatter}\n${content}`

    await this.fileGateway.writeTextFile(outputPath, fullContent)

    return { outputPath }
  }

  private buildFrontmatter(input: MarkdownOutputInput): string {
    const frontmatter: Record<string, any> = {
      title: input.source.name.replace(/\.[^/.]+$/, ''),
      aliases: [],
      tags: [],
    ...
    if (input.paperDirection) {
      frontmatter['paper_direction'] = input.paperDirection.mainDirection
      frontmatter['sub_directions'] = input.paperDirection.subDirections
      frontmatter['keywords'] = input.paperDirection.keywords
      
      // 将方向和关键词添加到标签中
      frontmatter.tags.push(input.paperDirection.mainDirection)
      if (input.paperDirection.subDirections) {
        frontmatter.tags.push(...input.paperDirection.subDirections)
      }
      if (input.paperDirection.keywords) {
        frontmatter.tags.push(...input.paperDirection.keywords)
      }
    }
    ...
    if (input.chunking?.enabled) {
      frontmatter['chunking'] = {
        enabled: true,
        chunk_count: input.chunking.chunkCount,
        max_tokens: input.chunking.maxTokens,
      }
    }
    ...
    for (const [key, value] of Object.entries(frontmatter)) {
      if (Array.isArray(value)) {
        ...
        for (const [subKey, subValue] of Object.entries(value)) {
          frontmatterStr += `  ${subKey}: ${typeof subValue === 'string' ? `"${subValue}"` : subValue}\n`
        }
      } else {
        frontmatterStr += `${key}: ${typeof value === 'string' ? `"${value}"` : value}\n`
      }
    }
    frontmatterStr += '---'
  ...
  private buildContent(input: MarkdownOutputInput): string {
    ...
    const truncationDetail = input.truncation?.applied
      ...
      : ''
    const usageLines = this.formatUsage(input)
    const chunkingLines = this.formatChunking(input)
    const paperDirectionLines = this.formatPaperDirection(input)
    const appendixLines = this.formatChunkAppendix(input)
  ...
  private formatChunking(input: MarkdownOutputInput): string[] {
    if (!input.chunking?.enabled) {
      return ['- 长文分块：未启用']
    }
    ...
    const lines = [
      ...
      `  - 最大Tokens: ${input.chunking.maxTokens ?? '未知'}`,
  ...
  private formatUsage(input: MarkdownOutputInput): string[] {
    if (!input.usage) {
      return ['- Token 统计：未提供']
    }
  ...
  private formatPaperDirection(input: MarkdownOutputInput): string[] {
    if (!input.paperDirection) {
      return []
    }
  ...
  private formatChunkAppendix(input: MarkdownOutputInput): string[] {
