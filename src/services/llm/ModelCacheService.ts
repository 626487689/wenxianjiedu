...
interface ModelCacheStatistics {
  hits: number
  misses: number
  hitRate: number
  evictions: number
...
export class ModelCacheService {
  ...
  private statistics: ModelCacheStatistics = {
    hits: 0,
    misses: 0,
    hitRate: 0,
    evictions: 0,
  ...
  private memoryThreshold = 5 * 1024 * 1024 // 5MB内存阈值
  ...
  private generateCacheKey(request: ModelRequest): string {
    ...
    const keyParts = [
      request.modelName,
      request.endpoint,
      request.prompt.substring(0, 100), // 使用前100个字符作为提示词的标识
  ...
  get(request: ModelRequest): ModelResponse | null {
    ...
    logger.info(`从缓存中获取模型结果，访问次数: ${entry.accessCount}`)
    this.updateStatistics(true, startTime)
  ...
  set(request: ModelRequest, response: ModelResponse): void {
    ...
    // 更新内存使用
    this.updateMemoryUsage()
    logger.info(`模型结果已缓存，token数: ${tokenCount}, 当前缓存大小: ${this.cache.size}, 内存使用: ${(this.statistics.memoryUsage / (1024 * 1024)).toFixed(2)}MB`)
  }
  ...
  clearByModel(modelName: string): void {
    ...
    for (const [key, entry] of this.cache.entries()) {
      if (entry.request.modelName === modelName) {
        this.cache.delete(key)
        count++
        this.statistics.evictions++
      }
  ...
  getStats(): {
    ...
    maxSize: number
    ttl: number
    statistics: ModelCacheStatistics
  } {
    return {
  ...
  private updateMemoryUsage(): void {
    ...
    for (const entry of this.cache.values()) {
      ...
      memoryUsage += entry.request.prompt.length * 2
      memoryUsage += entry.response.content.length * 2
      memoryUsage += entry.request.modelName.length * 2
      memoryUsage += entry.request.endpoint.length * 2
      memoryUsage += 8 * 5 // 5个数字字段，每个8字节
    }
